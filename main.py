...
def model_f(m, res=False):
    return 'model_' + get_user_id(m)
def create_model(m, res=False):
    if model_f(m) not in globals():
        globals()[model_f(m)] = ''
def edit_model(m, value, key, res=False):
    global d
    model_n = model_f(m)
    inf = "'" + value + "'"
    exec(model_n + ' += ' + str(inf), globals())
def edit_model_sell(m, value, key, res=False):
    model_n = model_f(m)
    inf = "'" + get_dict(m).get(key)[value] + "_'"
    exec(model_n + ' += ' + str(inf), globals())
def get_model(m, res=False):
    model_n = model_f(m)
    result_model = eval(model_n)
    return result_model
def del_model(m, res=False):
...
def storage_model(m, res=False):
    global d
    create_cache(m)
    edit_cache(m,m.text)
    if (get_cache(m).isdigit() == False) or (int(get_cache(m)) > len(d.get('Model'))) or (int(get_cache(m)) == 0):
        msg = bot.reply_to(m, 'Неправильный номер модели')
        del_cache(m)
        del_cache1(m)
        return
...
def storage_flavours(m,res=False):
    global d
    create_cache1(m)
    edit_cache1(m, m.text)
    if (get_cache1(m).isdigit() == False) or (int(get_cache1(m)) > len(d.get('Flavours'))) or (int(get_cache1(m)) == 0):
        msg = bot.reply_to(m, 'Неправильный номер вкуса')
        del_cache(m)
        del_cache1(m)
        return
...
def storage_amount(m,res=False):
    model = d.get('Model')[int(get_cache(m))-1]
    edit_cache(m, m.text)

    if get_cache(m).isdigit() == False:
        msg = bot.reply_to(m, 'Неправильное число:')
        del_cache(m)
        del_cache1(m)
        return
    if int(get_cache(m)) == 0:
        ...
        st[model][d.get('Flavours')[int(get_cache1(m))-1]] = int(get_cache(m))
    json_save_st()
    msg = bot.reply_to(m, 'Количество одноразок этой модели изменено на  '+ get_cache(m) + 'шт.')
...
def model(m, res=False):
    create_cache(m)
    edit_cache(m, m.text)
    create_model(m)
    line = list(st.keys())
    if (get_cache(m).isdigit() == False) or (int(get_cache(m)) > len(line)) or (int(get_cache(m)) == 0):
        msg = bot.reply_to(m, 'Неправильный номер модели')
        del_cache(m)
        del_model(m)
        return

    edit_model(m, line[int(get_cache(m))-1], 'Model')
    edit_cache(m, line[int(get_cache(m)) - 1])
    line = list(st[get_cache(m)].keys())
    msg = bot.reply_to(m, 'введите номер вкуса\n' + genlist(line))

    bot.register_next_step_handler(msg,flavours)
def flavours(m,res=False):
    create_cache1(m)
    edit_cache1(m, m.text)
    line = list(st[get_cache(m)].keys())
    print(line)
    if (get_cache1(m).isdigit() == False) or (int(get_cache1(m)) > len(line)) or (int(get_cache1(m)) == 0):
        msg = bot.reply_to(m, 'Неправильный номер вкуса')
        del_cache(m)
        del_cache1(m)
        del_model(m)
...
def amount(m,res=False):
    ...
    if st[Model][Flavour] == int(get_cache(m)):
        ...
        msg = bot.reply_to(m, 'Вы просите больше, чем есть на складе!')
        del_cache(m)
        del_cache1(m)
        del_model(m)
        return
    ...
    msg = bot.reply_to(m, 'вы взяли '+ get_cache(m) + ' одноразок на руки!')


@bot.message_handler(commands=["sell"])
def sell_cb(m, res=False):
    del_model(m)
    del_dict(m)
    create_dict(m)
...
def s_model(m, res=False):
    create_cache(m)
    edit_cache(m, m.text)

    if (get_cache(m).isdigit() == False) or int(get_cache(m)) > len(get_dict(m).get('Model')) or int(get_cache(m)) == 0:
        msg = bot.reply_to(m, 'Неправильный номер модели')
        del_cache(m)
        del_model(m)
        return
...
def s_flavours(m,res=False):
    global d
    create_cache(m)
    edit_cache(m, m.text)

    if (get_cache(m).isdigit() == False) or (int(get_cache(m)) > len(get_dict(m).get('Flavours'))):
        msg = bot.reply_to(m, 'Неправильный номер вкуса')
        del_cache(m)
        del_model(m)
        return
...
def s_money(m,res=False):
    ...
    msg = bot.reply_to(m, 'Вы пополнили казну мстителей на ' + get_cache(m) + 'руб. Поздравляем!')
